...
```
### 1.4 es-module-lexer
- [es-module-lexer](es-module-lexer)是一个JS模块语法解析器
```js
...
```
## 7.修改导入路径
- 修改返回的main.js中的vue的路径
    - `import { createApp } from 'vue'`
    - `import { createApp } from '/node_modules/.vite/deps/vue.js'`
- 请求`/src/main.js`
    - 执行`resolveId`
        - `resolvePlugin` 如果/开头表示是绝对路径，返回此文件硬盘年绝对路径`c:/project/src/main.js`,结束`resolveId`
    - 执行`load` 返回`null`
    - 执行`transform`
        - 执行`importAnalysisPlugin`
            - 获取依赖的`vue`,重新执行`PluginContext.resolve`
                - 执行`preAliasPlugin`,获取`vue`相对路径`/node_modules/.vite521/deps/vue.js` 把`vue`变为`/node_modules/.vite/deps/vue.js`
- 请求`/node_modules/.vite/deps/vue.js`
    - 执行`resolveId`
        - `resolvePlugin` 如果/开头表示是绝对路径，返回此文件绝对路径`/node_modules/.vite521/deps/vue.js`,结束`resolveId`
