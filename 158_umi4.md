...
- [layout](https://procomponents.ant.design/components/layout#Settings)
...
export async function signup(body?: API.SignupUser) {
  return request<string>("/api/user", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
export default function () {
  ...
  useEffect(() => {
    if (initialState?.currentUser) history.push("/");
  }, [initialState]);
  const onFinish = (values: any) => {
    run(values);
  };
...
export default function (initialState) {
  ...
  return {
    ...
    memberCan: [RoleCodes.root, RoleCodes.admin, RoleCodes.member].includes(
      role
    ),
  };
}
